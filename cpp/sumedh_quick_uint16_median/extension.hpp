...
PYBIND11_MODULE(extension, m) {
  ...
  m.def("median", &median_uint16_2d, py::arg("array"), py::arg("non_zero_threshold") = 0.5,
        "Return the lower median of a 2D uint16 NumPy array, ignoring zeros. Returns NaN if the proportion of non-zero elements is below the specified threshold or if there are no non-zero elements.");
}
